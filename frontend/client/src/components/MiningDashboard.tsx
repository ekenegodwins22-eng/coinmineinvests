...
export default function MiningDashboard() {
  const { data: earningsData, isLoading: earningsLoading } = useQuery<EarningsData>({
    ...
    refetchInterval: 5000, // Optimized to 5 seconds for better performance
  ...
  const { data: contracts = [], isLoading: contractsLoading } = useQuery<MiningContract[]>({
    ...
    staleTime: 60000, // Contracts don't change often - 1 minute stale time
    gcTime: 600000, // Keep in cache for 10 minutes
  });

  const { data: announcements = [], isLoading: announcementsLoading } = useQuery<Announcement[]>({
    queryKey: ["/api/announcements"],
    staleTime: 300000, // Announcements change rarely - 5 minutes stale time
    gcTime: 1800000, // Keep in cache for 30 minutes
  });

  const { data: transactions = [], isLoading: transactionsLoading } = useQuery<Transaction[]>({
    queryKey: ["/api/transactions"],
    staleTime: 30000, // Transactions updated occasionally - 30 seconds stale time
    gcTime: 600000, // Keep in cache for 10 minutes
  });

  const { data: withdrawals = [], isLoading: withdrawalsLoading } = useQuery<Withdrawal[]>({
    queryKey: ["/api/withdrawals"],
    staleTime: 30000, // Withdrawals updated occasionally - 30 seconds stale time
    gcTime: 600000, // Keep in cache for 10 minutes
  });

  // Fetch crypto prices for USD conversion - optimized frequency
  const { data: cryptoPrices = [] } = useQuery<any[]>({
    queryKey: ["/api/crypto-prices"],
    refetchInterval: 120000, // Reduced to 2 minutes for better performance
    staleTime: 90000, // Consider prices fresh for 1.5 minutes
    gcTime: 300000, // Keep in cache for 5 minutes
  });
  ...
  const btcPrice = cryptoPrices.find((crypto) => crypto.symbol === 'BTC')?.price || 0;
    ...
    <div className="space-y-8" data-testid="container-mining-dashboard">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <Card className="bg-cmc-card border-gray-700 relative overflow-hidden" data-testid="card-total-earnings">
          {/* Mining animation background - optimized for performance */}
          <motion.div 
          ...
          <CardContent className="relative z-10">
            ...
            <div className="mt-4 space-y-3">
              <div className="bg-cmc-dark rounded-lg p-3 relative overflow-hidden">
                ...
                {/* Micro pulse animation for active earning - optimized */}
        ...
        <Card className="bg-cmc-card border-gray-700 relative overflow-hidden" data-testid="card-mining-power">
          {/* Live mining animation - optimized */}
          {activeContracts.length > 0 && (
            <motion.div 
